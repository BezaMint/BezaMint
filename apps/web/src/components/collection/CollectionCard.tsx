    ...
    >
      ...
      <div>
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="badge-primary flex-shrink-0 capitalize">
            {category.replace('_', ' ')}
          </span>
        </div>
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-3">
            {tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-xs px-2 py-0.5 rounded-md bg-bezamint-muted/50 text-gray-500"
              >
                {tag}
              </span>
            ))}
            {tags.length > 3 && <span className="text-xs text-gray-600">+{tags.length - 3}</span>}
          </div>
        )}
      </div>
    </Link>
